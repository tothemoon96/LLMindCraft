...
#     MAX_JOBS=1 FLASH_ATTENTION_FORCE_BUILD=TRUE FLASH_ATTENTION_FORCE_CXX11_ABI=FALSE python setup.py bdist_wheel && \
#     cd dist && \
...
# RUN MAX_JOBS=1 pip install -U --no-cache-dir git+https://github.com/NVIDIA/TransformerEngine.git@v1.4
...
ARG CUDA=cu12
ARG PYTHON=cp310
ARG CV_CUDA_VERSION=0.7.0
RUN wget https://github.com/CVCUDA/CV-CUDA/releases/download/v${CV_CUDA_VERSION}-beta/cvcuda_${CUDA}-${CV_CUDA_VERSION}b0-${PYTHON}-${PYTHON}-linux_x86_64.whl \
    && pip install cvcuda_${CUDA}-${CV_CUDA_VERSION}b0-${PYTHON}-${PYTHON}-linux_x86_64.whl \
    && rm -rf cvcuda_${CUDA}-${CV_CUDA_VERSION}b0-${PYTHON}-${PYTHON}-linux_x86_64.whl
